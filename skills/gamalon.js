...
'card':	["i understand that you're talking about your card"],
...
for (x of path){
  ...
  if (dict[x]){
    ...
    // bot.reply(message, knownResp ? knownResp[Math.floor(Math.random() * knownResp.length)] : 'test response for bryan => welcome to match bot : )')
...
//
//     dict={ // note don't use empty arrays!!!
//       'unknown': ["sorry I didn't catch that.", 'wot wot?', "say again?", "can you speak more slowly please."],
//       // 'intent2': ['response1 for intent 2', 'response2 for intent2'],
//     }
//     knownResp = dict[intent]
//     bot.reply(message, knownResp ? knownResp[Math.floor(Math.random() * knownResp.length)] : 'test response for bryan => welcome to match bot : )')
//     bot.reply(message, intent);
//     bot.reply(message, `${confidence}`);
//     bot.reply(message, JSON.stringify(path));
